...
A repeatable, automated way to verify Delray's traffic totals against a trusted
reference, instead of relying on manual comparison with Sniffnet and the Windows
Task Manager on a user's machine.

## Problem

On Windows, Delray's interface totals can be orders of magnitude lower than the
...
2. Delray's raw-packet handling (strict parsing dropping frames, capture-thread
   backpressure), or
...
| Component | Role |
| --- | --- |
| `src/bin/refcap.rs` (binary `refcap`) | Minimal raw capture counter. Reads the same Npcap device as Delray, counts packets/wire bytes/IP bytes by EtherType, and reports Npcap's `dropped` / `if_dropped` stats every second. No protocol parsing, no attribution, no bounded channel. |
| `scripts/verify-capture.ps1` | Orchestrates one run: starts `refcap` + `delray` concurrently, generates traffic (iperf3 or SMB copy), samples Windows adapter counters before/after, computes three ratios, writes `report.txt`/`report.json`, returns a CI-friendly exit code. |
...
| `pipeline_ratio` | Delray `in_bytes+out_bytes` / refcap `bytes_ip` | Delray's total pipeline vs raw IP bytes that Npcap actually delivered. If low while `capture_ratio` is high → Delray-side loss: strict parser dropping frames, capture-thread backpressure (2 MB buffer + 8k bounded channel + 100 ms blocking sleep), or scope (non-IP, ARP). This is the ratio that will confirm/reject the "architecture design" hypothesis for the order-of-magnitude shortfall. |
...
- Windows test machine with Npcap installed; the script (and Delray/refcap) must
  run as Administrator.
- Built binaries: `target\release\delray.exe` and `target\release\refcap.exe`
...
`refcap.jsonl`, `delray.json`, `iperf*.txt`, `report.txt`, `report.json`.
...
| ≈ 1 | low | Delray-side: strict parsing drops and/or pipeline backpressure. Compare `ip_invalid_packets`; inspect Delray diagnostics for probe/lookup latency. This is the architecture hypothesis. |
...
- Loopback traffic is not covered; Npcap loopback capture on Windows has
  separate adapter/direction quirks (Delray intentionally restricts loopback to
  inbound).
...
| Scenario | Load | refcap/adapter | Delray/refcap-IP | Npcap dropped | Verdict |
| --- | --- | --- | --- | --- | --- |
...
mismatch between Delray and Sniffnet in the original run (compare the
`\Device\NPF_...` GUIDs), an older Delray build, or measurement-scope
differences.

Since this verification, Delray's diagnostics JSONL also reports pcap-layer
