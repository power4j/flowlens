...
进程维度与守恒摘要的窗口口径：60 秒桶 × 5 桶 = 5 分钟滚动窗口，复用 IP 维度的 epoch bucket 机制。进程列表的 Recv/Sent/Total 与 top-N 排序使用窗口口径，窗口字节为零的历史大户不再占位；累计字节保留在进程详情与 plain/JSON 报表（Win 列），两个口径互不覆盖。
